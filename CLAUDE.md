...
## 🎯 Current Focus
**IN PROGRESS — Phase 39: Cookie Jar (`cjar`), game 18.** Built task-by-task from `docs/superpowers/plans/2026-08-02-cookie-jar.md` (Tasks 1–16 of 17 done) against the confirmed spec `docs/new-game-tech-cookie-jar.md`. **Code-complete at SW v157 — zero stubs — but the phase gate is NOT closed.**
...
**What remains, and why the snapshot still cannot say "complete":**
1. **The three-device session** (plan Task 15 Step 10) — **never run, and now the ONLY blocking item.** No harness substitutes for it; PKO's `PKO_CARRION_OPEN` is the precedent for recording an unverified path honestly rather than implying coverage. Round 1 never got past flip 1 (BUG-06) and round 2 was a single-device check, so **nothing downstream of the decision window has been seen live on more than one device.**
2. ~~The offline install check~~ — **DONE, 3 Aug 2026, PASSED.** Unregister → hard reload → Offline → "See the Cards" → illustrated cards appeared. The 14 core-art JPEGs and the manifest are precached.
3. ~~Task 17's docs~~ — **DONE** (`code-map.md`, `game-identities.md` § Game 18, `docs/phase39-snapshot.md`, the `frt-implementation-notes.md` entry for Delta 2, decision-log, brief-prompt roster). Committed as `ee231f7`. The snapshot needs a refresh for round 2 but exists.
...
**Playtest round 2 ran 3 Aug 2026 — SW v160. The offline install check PASSED and is now CLOSED.** Unregister → hard reload → tick Offline → open Cookie Jar → "See the Cards": illustrated cards appeared, so the 14 core-art JPEGs and the manifest are precached. **The three-device session is still the one unrun item.** Round 2 produced five changes, all shipped at v160:
1. **BUG-08 — the "card stuck under the next one" was the Treat, and nothing was leaking.** `cjarRenderStage` appended the deck's face-down card *and* `cjarCounterTreat` into the **same 56 px column**, so the unclaimed Treat rendered as an unlabelled thumbnail wedged under the deck, in the slot that means "next". Different each Raid because the Treat is scheduled per Raid; persistent across flips because that is its rule. The render loop was correct; the Treat had no home. It has its own labelled slot now.
2. **DD-12 — the stage is THREE BANDS**, superseding DD-11's single row in part. The arithmetic: on a 390 px phone the row left the history **142 px, ~2.6 thumbs of a 10+ flip Raid**, and `strip.onclick` fought the swipe that would scroll it. Now: table state (Crumbs) → the decision (just revealed + next + Treat) → full-width scrollable history. New `.cjar-card-next` (6.5rem) sizes the deck near the *live* card, not near the spent ones — the object of the bet was rendering smaller than the thing it was being compared to.
3. **Crumbs moved out of the private strip.** `Crumbs · Cookie Stash · This Raid` read as three identical chips, and one of the three is shared table state while the other two are personal — which is exactly why nobody could tell what they were. Crumbs went to the stage; the two that remain are now told apart by **shape** (filled = banked and safe, outlined = provisional and at risk).
4. **DD-13 — settings pills carry the thematic name; the VALUE is a live line below the row.** Decision Time had shipped with its seconds written nowhere in the app; Match Length baked `(3)`/`(5)` into its labels. `ui-style.md` § Settings Card Standard now carries the rule (required when the option encodes a concrete value not in its label). **The other 17 games are NOT swept** — deliberately a separate pass.
5. **DD-14 — the card gallery is now a How to Play TAB**, `cjar-cards-overlay` deleted. `ui-style.md`'s How-to Overlay Standard gained an **optional tab bar**, with the line drawn explicitly: **teaching material earns a tab; a mid-play reference (PKO's chain) keeps its own overlay.** "See the Cards" on the menu lands on the tab directly, so the offline check is unchanged.
Gameover also gained a title — **The Haul 🍪** — matching the suite's "The [Noun]" shape (PKO's The Hierarchy, FLW's The Vault), with the nursery rhyme demoted to subtitle.

**One question raised in round 2 and deliberately NOT acted on.** In the base game flip 1 auto-resolves before any decision (correct — Incan Gold; you cannot decline to enter the temple). In **Dibber Dobber the first decision of every Raid is made on a completely empty stage**, and `cjarBuildDeck`'s Sylly branch returns *before* `cjarFloatCookies`, so **Snack Friendly does nothing in Sylly** (its settings card is hidden, so this is honest, not a lie). That makes the first DD commit a blind gamble that can cost 4 of your 5 starting cookies. Delta 7 accepted the asymmetry as unavoidable; it isn't quite — floating a cookie to the front of the Sylly deck would keep the commit blind while making it unpunishable, exactly what Snack Friendly does for the base game. **Left alone because it is a rules change to an economy whose balance baseline (DD-06) was measured without it**, and the project's own discipline is not to retune pre-playtest. Owner call.
...
**Procedure (v159 onward):** DevTools → Application → unregister the SW → hard reload (v159 installs fresh) → tick **Offline** → open Cookie Jar → **"See the Cards"** on the game menu. Illustrated cards = precached. Emoji = it did not. You never start a match.
...
**SW Version:** v160 (**Cookie Jar (`cjar`) — game 18. Playtest rounds 1–2 fully triaged; the offline install check PASSED and is closed. The three-device session is the ONLY unrun item.** Simultaneous-choice push-your-luck, MDLM-only, **3–8** players (min dropped from 4, owner call 3 Aug 2026 — 3-player balance unsimulated), host-authoritative, host-as-participant. **Base mode is Incan Gold 1:1** — reveal → resolve → choose about the *next, unseen* card, with a bust ending the Raid before any decision window (Delta 3). **Sylly Mode = Dibber Dobber** preserves that same mental model by inverting the implementation: choices commit **blind**, then the card is revealed and resolved against them (**Delta 7**, owner call 2 Aug 2026 — spec §6.3 originally resolved against the face-up card, which made Sylly a different game). Three actions (Take / Play Innocent / Dob), no bust, nobody leaves; one running `cjarStashes` seeded **once** with 5. New files: `js/games/cjar.js` (1777 lines, zero stubs), `data/cjar-data.json` (15 cookie values totalling 124, 3 tier bands, 5 family archetypes × 4 warn + 4 bust lines, 5 treats, both schedules), `docs/cjar-content-guide.md`, `data/art/cjar/` core art (**14 JPEGs, 18.7 MB → 492 KB**, every file under the 40 KB ceiling), six screens, six overlays. **Five headless tools**, all green: `verify-cjar-deck.js` **73** · `verify-cjar-loop.js` **102** · `verify-cjar-dd.js` **47** · **`verify-cjar-loopback.js` 110** (host↔client over a Firebase-shaped wire, with a DOM of real mock elements — the only one that executes a single line of render code, and the one that caught BUG-06) · `simulate-cjar-dd.js` (balance instrument — asserts nothing, always exits 0). Brand **honey-gold `#D4A017` with DARK ink** — white measures 2.38:1 against it, so `ctaTextClass: 'text-stone-800'` (second consumer after FRT) and labels use the darkened `#7A5C0A`. Two engine-side lessons elevated to `logic-engine.md`: a readiness gate reading a per-seat array must be tested in the mode where that array is **empty** (`[].every()` is `true` — BUG-05 let the host resolve on the first tap in Sylly), and a **two-device loopback** should prove the packet contract before the real multi-device session (it found two defects that had passed 222 green checks). **The offline install check PASSED at v160 and is closed; the three-device session remains the only unrun item.** Rounds 1–2 are both fully triaged (BUG-06 through BUG-08, DD-07 through DD-14); TG-04 is closed. One rules question is open and deliberately unactioned — the blind first flip in Dibber Dobber (see § Current Focus). Full history for v137–v159: `docs/sw-changelog.md`.)

**Previous versions:** v155 and earlier — `docs/sw-changelog.md` (the outgoing entry moves there on each bump; only the current version keeps its notes here).

...
**Playtest round 2 ran 3 Aug 2026 across two passes — SW v158 → v161. The offline install check PASSED and is now CLOSED.** DevTools → unregister → hard reload → Offline → Cookie Jar → How to Play → The Cards tab: illustrated cards appeared, so the 14 core-art JPEGs and the manifest are precached. **The three-device session is still the one unrun item — every round-2 pass has been single-device.**

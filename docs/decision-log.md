...
Category: Strategy
Decision: Three things at once, all surfaced by scoping the lobby redesign. (1) The prohibition list ($0, no build, no backend, no libraries) is reorganised into a **three-tier cost envelope** — `docs/cost-envelope.md` — which prices additions instead of forbidding them, and splits what used to be one box: **craft constraints** (44 px, transform/opacity-only motion, reduced-motion, silent offline failure) are permanent and never traded, while **business constraints** (zero cost, no backend, no build step, install size) are genuinely reviewable. (2) The owner's strategic direction is recorded as *direction, not commitment*: heading toward eventual **monetisation**; the **product is the games**, not art/skins — the answer that settles the tiers, since it keeps bitmap art a cost centre rather than a storefront; **persistent accounts expected**, with the controller doubling as the player avatar. (3) The 2026-06-30 no-build deferral's **revisit trigger has fired**: `index.html` is 763,121 bytes and the suite is at 20 games — both at or past the stated ~750 KB / ~20 games line.
Why: The prohibition framing had started producing bad answers, because the project has already walked through several of those doors deliberately (Three.js vendored and shipping, Firebase running every multiplayer room, six games carrying bitmap artwork). Measuring rather than asserting reframed it: the install is **11.49 MB**, of which `data/art/` alone is **6.03 MB for six games**. Visual ambition expressed in *code* is therefore nearly free — Three.js is a sixth of one game's artwork — while ambition expressed in *pixels* is the only real cost centre.
Changed: new `docs/cost-envelope.md`; `docs/token-budget-register.md` § 5 Lever A marked TRIGGER FIRED with the measurement, plus a third option the 2026 decision never weighed — **runtime assembly** (each `js/games/[abbr].js` owns its own markup, as it already owns state, logic, screens and packets). That is not a build step, so it dodges the owner-fragility objection that deferred Lever A; its real cost is a boot-ordering problem, since `js/engine.js` calls `getElementById` at parse time. Deferred: the build-step spec task itself — now due, not started, and deliberately **not** folded into the lobby redesign.
Detail: `docs/cost-envelope.md` (§ 2a craft vs business, § 4 the tiers, § 5 precache vs runtime-cache, § 7 the strategic answers); the original deferral is this log's 2026-06-30 entry.
...
Why: The "sort by colour" lobby mode made per-game colour drift visible, and the pills had accreted three different dark-ink exceptions that each passed a contrast check in isolation but read as inconsistency side-by-side. The owner's call: one *consistent* scheme per game beats a scheme that's individually legible but collectively incoherent — a barely-passing swatch was accepted knowingly (the FRT-yellow precedent) rather than fought per-game.
Changed: `css/styles.css` (`.pill-active-{frt,amber,cld,comb}`, `.game-toggle-on-{frt,amber,cld,comb}`, `.comb-cta`, `.cld-cta` → `#fff`; deleted 4 `#btn-*` / `#btn-*-menu-play` white-ink ID overrides), `index.html` (~20 YGI + 7 FRT static buttons `text-stone-800` → `text-white`; `btn-ss-tiebreak-go-second` stone→teal to match its pair), `js/games/frt.js` (3 button sites drop `FRT_INK`), `js/engine-multiplayer.js` (`ctaTextClass` removed from `ygi`/`frt`/`comb`). Docs: `ui-style.md` § Action Button Standard + § Settings Layout Standard + § Menu Title Treatment note, `docs/rules/per-game-classes.md`, `CLAUDE.md` Quick Index. Deferred/superseded: the main-menu *title-screen* button polish (gel-btn / two-tone titles) is explicitly out of scope for this pass, still owner-undecided.
Detail: `.claude/rules/ui-style.md` § Action Button Standard → "Locked per-game button scheme"; `docs/sw-changelog.md` v227.

---

## 2026-09-10 — Honeycomb Hills' first real multi-device session: meadow UX pass + a P0 fix (SW v226)
Category: Architecture
Decision: The first 3–4 device session found one hard break and a cluster of UX gaps. Fixed: the **roll-7 strand** (every 7 froze every client on the default settings — `combBeginSeven()`'s no-owe branch advanced the host and broadcast nothing); and reworked `screen-comb-meadow` — a floating **player panel** (active row lit = the turn-handover signal), a **persistent roll result** + **11-tick probability ruler** (replacing the invisible pog pips), a bottom **player-stats strip**, a **65% board focus-dim** post-roll and during placement with own-colour target glow, blinking **Trade Blossom beacons** (a cheap ~18 fps RAF, the only meadow-screen animation), the Drone Cell drawn visibly smaller than the Queen Dome, and `comb-map-overlay` rebuilt as a **3-zone gesture-locked detail view**. All new UI reads existing public state; the one schema change is `COMB_OVERFLOW_DONE.spilled` (bool).
...
Why: `getMaxPlayers`/`getMinPlayers` are consulted only while a room fills — before the game has shown a screen — so five games' bounds resolved against a setup variable still at its default, capping rooms below their own Pass-the-Phone range. And a rule *titled* MDLM read as someone else's problem to the two TLM games, which is why eight games shipped with no quit teardown at all. Both classes were invisible to all 14 existing harnesses, because every one of them tests a game's rules and nothing tested the declarations layer above them.
Changed: `js/engine-multiplayer.js` (5 games' bounds → constants; `mpNotifyPlayerLeft` + generic `MP_PLAYER_LEFT`; `mpRosterNeedsBalance` + balance gates on the host CTA and roster confirm), 8 game plugins' quit-confirm handlers (li5, gm, ss, jec, ygi, lttp, nat, dsd), `index.html` (`#mp-roster-hint`), `sw.js` v209 → v210, **new `tools/verify-mp-configs.js`** (18 games; takes `MP_SRC=`, fails on pre-fix `main`), `logic-engine.md` (§ Mid-Game Quit Contract re-scoped + rewritten; MP_GAME_CONFIGS schema gains the bounds rule and `requiresBalancedTeams`), `code-map.md`, 8 identity docs' T7c, `shared-implementation-notes.md` § Multiplayer Lessons ML-01…ML-04, `deferred-work.md` (9 bullets closed). **Scope grew on evidence, twice:** the identity pass named 5 quit games and 4 bounds games; grepping for the shape found **8 and 5** — LTTP, NAT and DSD had the quit gap unrecorded, and DSD the bounds gap that JEC's entry had asked someone to check. **Deferred:** NAT's PTP floor (pills 4–8 vs lobby min 3) is a design call, not a bug — the harness prints it as a documented note; the ten pre-existing per-game `[ABBR]_PLAYER_LEFT` implementations are deliberately not swept.
Detail: `docs/implementation-notes/shared-implementation-notes.md` § Multiplayer Lessons (ML-01 the timeline trap, ML-02 completeness ≠ validity, ML-03 mode-scoped rules, ML-04 the untested declarations layer), `docs/deferred-work.md` top section.

---

## 2026-08-23 — Identity-doc migration COMPLETE; `game-identities.md` deleted
Category: Process
Decision: All 18 games now have a per-game identity doc at `docs/game-identities/[abbr].md`; `docs/rules/game-identities.md` is deleted. Routing is now explicit and reciprocal — **identity doc = the non-technical source of truth** (pitch, premise, theme, terminology, settings, every UI string, Sylly Mode, art, table shape); **`docs/code-map.md` = the technical map** (screen/overlay IDs, state variables, key functions, MP packet tables). Neither duplicates the other, and each names the other.
Why: Closes the 2026-08-22 decision's deferred half (17 passes + final cleanup). The one-way routing that remained — CLAUDE.md knew the split, but an identity doc opened on its own didn't — was the last real gap: these docs are meant to be handed to an artist or reviewer standalone, so each now carries a header pointer to `code-map.md`.
Changed: 18 identity docs (all passing `node tools/verify-identity-docs.js`, 1,787 verified copy strings; each gained a "Where the technical detail lives" header block). `docs/rules/game-identities.md` **deleted**. Nine live docs re-pointed per spec § 8.5 — `CLAUDE.md` (tracker column removed, six refs rewritten), `.claude/rules/definitions.md`, `.claude/rules/ui-style.md`, `phase-audit.md`, `new-game-checklist.md`, `new-game-process.md`, `new-game-technical-template.md`, `testing-session-protocol.md`, both token-budget files, plus four dangling refs in `code-map.md`. Historical records (impl-notes, tech specs, phase snapshots, superpowers specs/plans) deliberately left frozen — editing them would falsify the archive. **Deferred, found-not-fixed:** 22 findings across 9 of the 18 games, all in `docs/deferred-work.md`. Two recur enough to deserve one shared fix rather than nine patches — the **MDLM/TLM mid-game-quit contract** (GM, SS, YGI, JEC, and LI5 in TLM; note `logic-engine.md` writes the contract for MDLM only, so the rule needs widening) and the **lobby player-count cap** pinned to a game-local variable only the single-device setup screen ever sets (LTTP, YGI, JEC, SS).
Detail: `docs/superpowers/specs/2026-08-22-game-identity-docs-design.md` (now marked COMPLETE), `docs/deferred-work.md` (the per-game findings), `docs/implementation-notes/shared-implementation-notes.md` § Template Gaps (the stale-bug-index lesson).

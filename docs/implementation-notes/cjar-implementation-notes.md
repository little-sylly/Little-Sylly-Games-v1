...
four supporting surfaces (headings, the score table, the family strip, gameover) came out of that
rework inconsistent with each other in ways that read as unfinished.

**DD-25 — the reveal choreography roughly doubles, and the settle tail overlaps decision time.** The three beats that carry information the player needs before deciding (flip, hold, payout) stay fully blocking and roughly double: `CJAR_FLIP_ANIM_MS` 2100 → **3200 ms** (flip 300→600 ms, hold 600→1200 ms, payout 700→1400 ms). The fourth beat (the spent card sliding into the trail, the next card rising from the deck) carries no information, so it moves OFF the blocking clock entirely — it plays as an unawaited ~1000 ms CSS-only tail (`.cjar-card-flipin` 550 ms, `.cjar-trail-settle` 500 ms) that fires once the blocking handover ends and overlaps the decision window that already opened. Same pattern as the delta-token layer's "transient animations float over a relative anchor, contribute zero layout height" rule. Within payout specifically, `cjarFlyTokens` now holds the value on screen for 500 ms before the token burst starts moving (`animationDelay = 500 + k*45`), and the CSS flight duration itself roughly doubles (380ms → 760ms, `animation-fill-mode: both` so the JS delay actually hides the token rather than showing it already-resting). **Honesty on the budget, not a time-neutral claim:** doubling flip+hold+payout alone (3200 ms) already exceeds the OLD total including settle (2100 ms) — freeing up the settle beat doesn't fully offset the doubling. Net effect is roughly **+1.1 s more blocking dwell per flip**, on top of DD-19's own +300 ms — an owner-accepted smoothness trade-off. If a future playtest says it drags, the documented lever is shifting more of hold/payout's length into the free settle tail rather than shortening the choreography outright.

**DD-26 — all three stage-column headings move to the top; nothing renders below the art.** Column 1 ("Up for Grabs") already read top-down; columns 2 and 3 read bottom-up (content → heading), so the row didn't scan as one unit, and column 2's label wrapped to two lines doing it. Column 2's `#cjar-stage-label-now` moves above `#cjar-table-hero`; its face-down copy shortens **`'next out of the jar'` → `'Next from Jar'`** to fit one line (the face-up `'just revealed'` text and the swap mechanism are unchanged — only DOM position and the face-down string moved). Column 3's label moves above `#cjar-deck-badge` and becomes **`'Left in Jar'`** (was `'the jar'`); the column's flex changes `justify-between` → `justify-center` so the deck-stack graphic centres in the remaining height instead of pinning to the top with dead space below. All three headings now share type scale and top position; colour stays deliberately non-uniform — col 1/2 keep the branded `cjar-label` gold, col 3 keeps muted `text-stone-300` (DD-24 already established the deck as the de-emphasised reservoir; this round fixes its position, not its emphasis).

**DD-26a — column 1 density.** `#cjar-grabs-card`'s padding/gap tightened (`px-2 py-2 gap-1` → `px-3 py-3 gap-1.5`) so it stops reading as mostly whitespace; `#cjar-crumbs-value` enlarged `text-2xl` → `text-3xl`; `#cjar-grabs-caption` — the specific visibility complaint, shipped at `text-[0.55rem]`, smaller than every other stage label — enlarged to `text-xs` with `leading-snug px-0.5`. No layout restructure, purely a type-scale fix.

**DD-27 — trail cards are individually tappable; the strip stays a pure scroll container.** Click handlers go on each card thumb in `#cjar-trail-strip` (`el.onclick = () => cjarOpenCardView(card)`, a plain property rather than `addEventListener` so it's directly callable from the headless harness), never on the container — the container's existing "no handler here, it fights the swipe" comment stays true, because a tap on a small discrete target and a drag across the whole strip are already distinguished by the browser's native click-vs-scroll heuristic on an `overflow-x:auto` container; a container-wide handler was the actual conflict, not a per-thumb one. Tapping a thumb opens the new `cjar-card-view-overlay` — `cjarOpenCardView(card)` renders an enlarged `cjarRenderCard(card,{size:'hero'})` (reusing the pre-DD-18 `cjar-card-hero` footprint, otherwise unused since column 2 shrank) plus the card's display name, closed by `btn-cjar-card-view-close`, same Decision-Modal shell as `cjar-tip-overlay`. Registered in `resetToLobby()`'s CJAR teardown array.

**DD-28 — score table gets a header row, a Status column, and a real grid layout.** A new `#cjar-reveal-header` row reads **Rank | Player | Stashed | Status | At Risk**, hidden during `'revealing'` (which keeps its own outcome-line format). Every standings row moved off `flex justify-between` — which stretches to fill the row regardless of content length, the actual source of the "lots of white space between name and stashed" complaint — onto `.cjar-reveal-grid`, a shared `1.8rem 1fr 4.6rem 4.6rem 4.2rem` column template so widths stay identical across rows regardless of name length. The old inline 🚪 emoji (a text character appended to the name string — not a button, so it fell outside the Aug 2026 Action Button Standard sweep entirely) is replaced by a proper Status badge: `.cjar-status-in` "Still In" when `cjarActive[i]` is true, `.cjar-status-out` "Snuck Out" when false — base game only, same restriction as At Risk (Dibber Dobber has no leave/still-in concept, so its rows leave both grid cells empty — the track stays, the content doesn't, keeping column widths matched without a conditional template).

**DD-29 — family strip spacing.** `#cjar-warning-strip`'s inter-slot gap tightened `gap-1.5` → `gap-1`; the gap between the strip and `#btn-cjar-family-tip` widened `gap-1.5` → `gap-2.5`, so a highlighted slot's `ring-2 ring-offset-1` no longer visually crowds the `[?]` button. Positional only — no change to what makes a slot "highlighted."

**DD-30 — gameover polish: fixed-slot medals, raid-history highlight.** The podium row dropped its 🍪-prefix-on-1st-place-only convention (the actual bug behind "it pushes rows out of alignment" — only one row had a leading glyph at all) for the suite's existing medal convention (🥇🥈🥉, already shipped in FRT/GTH/JEC/NAT/PASS/YGI) inside a **fixed-width leading slot** (`.cjar-medal-slot`, `width:1.4rem`) present on every row, blank past 3rd — so every row's text starts at the same x position regardless of rank. The raid-history grid now highlights the top scorer's cell per Raid column (bold + `cjar-label` gold), computed once per column (`Math.max` over the column) with no highlight on a tie. Both `btn-cjar-go-new` and `btn-cjar-go-leave` sized to `min-h-14 … text-xl font-semibold` (see DD-31).

**DD-31 — same-screen action buttons match in size and weight, no exceptions.** New standing rule in `ui-style.md` § Universal Menu Standard, applied to CJAR first: buttons representing real, distinct choices on the same screen must match in size and weight — including retiring the previously-documented "Back to the Box steps down because it's navigation" carve-out on the game menu's type scale. `btn-cjar-go-leave` and `btn-cjar-menu-back` both resized `min-h-11 text-sm`/`text-base` → `min-h-14 text-xl font-semibold`. The 17-other-game sweep is **deferred** — logged in `docs/deferred-work.md` alongside the other pending suite-wide sweeps (DD-13 settings dynamic-value line, DD-14 card galleries, the Decision Modal button-sizing divergence). The DD-30 medal-in-a-fixed-slot podium shape is also written into `ui-style.md` as a standing pattern for any future gameover podium wanting rank icons — prospective, not a retroactive sweep of the existing FRT/GTH/JEC/NAT/PASS/YGI podiums.

**DD-32 — stage headings share a literal grid row; the score table stops leaking the Stack's inter-zone gap; the Raid Complete screen becomes a full ledger.** *(Owner playtest of the shipped stage-polish round, 9 Aug 2026.)* Three findings, all presentational:

1. **Stage column balance.** DD-26 moved all three headings to the top of their own column and gave each the same type scale, but "top of the column" was still per-column (`justify-between` for col 1's padded white card, `justify-center` for col 2/3's plain columns) — not the same y-position, because col 1's card padding and col 2/3's vertical centring computed differently against the grid-stretched row height. Fix: the three headings ("up for grabs" / `#cjar-stage-label-now` / "Left in Jar") moved OUT of their columns into one shared `grid grid-cols-3` row directly above `#cjar-stage-row`, so alignment is structural, not a per-column style match. `#cjar-grabs-card` lost its `bg-white rounded-2xl shadow-sm px-3 py-3` — col 2/3 already carry visual weight from real card art (their own shape/shadow); a third layer of chrome around col 1's plain text+icon was what made the row read as unbalanced, not something col 2/3 needed to gain.
2. **Score table header distance.** `#cjar-reveal-header` and `#cjar-reveal-rows` were two direct children of the screen's own Stack column (`gap-4`, 16px) — so the Stack's own inter-zone spacing landed *between the header and its own table*, on top of the header's `mb-1` (4px), for a ~20px gap where row-to-row is 6px (`gap-1.5`). Root cause confirmed by measuring `getBoundingClientRect()` in a live render: `headerRectBottom` to `rowsRectTop` was exactly 20px, matching `gap-4 + mb-1`. Fix: wrapped both in one `flex flex-col gap-1` div so they read as one Stack zone. Also right-aligned the Stashed/At Risk header labels and switched their pills from `justify-self-start` to `justify-self-end` (previously both header and pills left-packed inside generously-sized fixed tracks, leaving an uneven gap before "At Risk"); Status text centred to match.
3. **Raid Complete screen (`cjarShowRaidSummary`) redesigned from a flat per-player list into a ledger table** — Player | current Stash | one column per Raid played so far (`R1`…`Rcurrent`), signed deltas (`+3`/`−2`), green/red/grey by sign, current-Raid column and the per-raid max cell both bold-highlighted (same max-per-column rule as the gameover history-grid, reused, not shared code — the gameover table intentionally has no sign/colour treatment, since a final tally's direction is already obvious from rank). `cjarRaidHistory[cjarRaidNo-1]` is already written by `cjarEndRaid` before this function runs, so no new state or packet — pure render. `#cjar-summary-rows` changed `flex flex-col gap-2` → `overflow-x-auto` for the wider table at 5 Raids.

Verified with a seeded 4-player render (`cjarLoadData()` + direct global assignment, bypassing the real MDLM lobby — CJAR is `multiplayerOnly`, no single-device path) driven through Playwright screenshots of all three screens before/after. `verify-cjar-loopback.js`'s `headerCols()`/`pillTexts()` assertions read `.textContent` only, unaffected by the added alignment classes — all four harnesses re-run clean, no count change (177/76/102/47), consistent with a presentational-only round.

**DD-33 — Treat cards never showed their point value.** *(Owner playtest, 9 Aug 2026.)* `cjarRenderCard` has always overlaid a cookie card's numeric value as a `.cjar-card-value` text plate on top of its tier art (the mechanism that lets 15 cookie values share 3 tier assets) — but the same treatment was never extended to treat cards, in either the asset-art branch or the emoji-fallback branch. A Treat sitting in `#cjar-treat-slot` on the Stage, in the trail strip, or opened in `cjar-card-view-overlay` showed only its picture and name; the only place its point value (5 for the three `special`-tier treats, 10 for the two `super`-tier treats) was visible anywhere in the live app was the How to Play card gallery's caption text (`${t.name} · ${t.points}`), which a player mid-Raid has no reason to have open. Fix: both branches now look up the treat's `points` from `CJAR_DATA.treats` and append the same `.cjar-card-value` div cookies already use — no new CSS, the class already positions correctly at every card size (`counter`/`thumb`/`stage`/`hero`) because `.cjar-card` is `position: relative`. Verified visually at `counter` size (both branches) and `hero` size via the `visual-check` skill; all four harnesses re-run clean (177/76/102/47 — unchanged, pure render/no state).

**Verification note:** all five harnesses re-run after this round; `verify-cjar-loopback.js` (the only harness with real render-executing mock elements) grew **147 → 177** — DD-26's label swap, DD-27's popup, DD-28's header row + status column (both modes, both Open Book settings), and DD-30's medal slot + raid-history highlight all needed a new assertion. `verify-cjar-deck.js` (76), `verify-cjar-loop.js` (102) and `verify-cjar-dd.js` (47) were untouched — nothing in this round touches the data layer, the base-game loop, or Dibber Dobber's ledger. `simulate-cjar-dd.js` output is unchanged within the existing noise band, consistent with a presentational-only round (DD-06's flagged Innocent-lean untouched).
...
**TG-10 — a documented "exception" in a style rule is itself a bug waiting for a second data point.** *(8 Aug 2026)*
`ui-style.md`'s Universal Menu Standard carved out "← Back to the Box" as deliberately smaller
than its three siblings, reasoned as "it is navigation, not an action." That reasoning didn't
survive contact with a real screen: CJAR's gameover pair (`btn-cjar-go-new` / `btn-cjar-go-leave`)
put a full-size primary next to an undersized secondary, and it read as unfinished rather than as
a deliberate hierarchy — the visual weight difference didn't communicate "this one is navigation,"
it just looked like a mistake. DD-31 retired the carve-out suite-wide: same-screen buttons
representing real, distinct choices now match in size and weight, no exceptions. **The
generalisable check:** when a style rule includes a hand-written justification for an exception
("because X is different"), that justification is a hypothesis, not a settled fact — it should be
re-examined the first time a *second* screen puts that exception's case next to its sibling and it
looks wrong, rather than trusted indefinitely because it was written down once. A rule with an
exception is more fragile than a rule without one; it survives review, not authorship.

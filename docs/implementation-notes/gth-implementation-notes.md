...
**Bug: Shrink-intro "Open the Case Files" button inert in single-device mode and with Sylly Mode (playtesting — FIXED)**
- Symptom: Tapping "Open the Case Files →" did nothing in single-device play, and also in MDLM when Sylly Mode was on.
- Root cause: `gthShowShrinkIntro()` used `const isHost = syllyMultiplayerMode === 'host'` — which is `false` for single-device mode (`'single'`). Consequently the button was hidden and the listener returned early for all non-`'host'` cases, meaning single-device was silently broken. The bug also manifested with Sylly Mode because that toggled no separate path, but the `=== 'host'` check excluded `'single'`.
- Fix: Changed to `const isHost = syllyMultiplayerMode !== 'client'` so that both `'host'` and `'single'` show the button and pass the listener guard.
- Lesson: Any host/authority check in a game that supports both PTP and MDLM must use `!== 'client'`, never `=== 'host'`. `=== 'host'` silently excludes the single-device path.

**Bug: Phase 2 stuck — all cases closed but Big Reveal never reached (playtesting — FIXED)**
- Symptom: The drawing timer ran out, all players showed ✓ Done on the Case Report screen, but nobody advanced to The Big Reveal. Host and all clients were stuck indefinitely.
- Root cause: `gthPhase2Complete` was declared as a state variable but never set to `true` anywhere in the codebase. In the timer callback, after `gthSubmitDiagnosisBatch()` returned, `gthShowCaseReport()` was called unconditionally. However, when all players had already submitted before the timer expired, `gthSubmitDiagnosisBatch()` had internally triggered `gthResolveScores()` → `gthShowBigReveal()`. The subsequent unconditional `gthShowCaseReport()` call overrode that navigation, routing every device back to the stuck case-report screen.
- Fix: (1) Added `gthPhase2Complete = true` at the top of `gthResolveScores()` before it broadcasts `GTH_FINAL_SCORES`. (2) Guarded the timer callback's `gthShowCaseReport()` call with `if (!gthPhase2Complete)`. The flag is reset to `false` in `gthResetState()`.
- Lesson: A state variable declared but never written is a reliable tell of a missing completion gate. Any "all players done → advance" pathway needs a flag that prevents a later code path from overriding the already-taken transition.

**Design: Progressive tremor for Stroke or Genius (CanvasDraw API extension — IMPLEMENTED)**
- Change: The `setTremor()` interval was previously static — fixed amplitude (±5px) and fixed frequency. It was too subtle to create meaningful pressure.
- New behaviour: `setTremorScale(progress: 0–1)` API added to `CanvasDraw`. Called on every countdown tick with `(total − remaining) / total`. Amplitude scales 5px → 20px; effective jiggle interval scales 1500ms → 150ms as `progress` approaches 1. Implementation uses a fast 100ms poll with `_lastTremorFire` timestamp gating rather than creating a new interval on each tick. The wrapper element reference is held in `_tremorWrapperEl` so `setTremorScale()` needs no argument.
- Lesson: Timer-driven progressive effects should use a single fast-poll interval + timestamp gate rather than recreating intervals on each tick. The 100ms poll keeps CPU overhead trivial; the gate controls the actual effect frequency.

**Design: No Limit drawing time option (IMPLEMENTED)**
- Change: Expression Window options changed from 20s / 30s (default) / 45s → 30s / 60s (default) / No Limit. No Limit is stored as `gthDrawingTime = 0`.
- Behaviour: When `gthDrawingTime === 0`, the countdown timer displays `∞` and returns without starting an interval. The drawing screen has no auto-submit — players tap "Done" manually. `setTremorScale()` is only called when `gthDrawingTime > 0` (No Limit draws skip tremor scaling because there is no progress fraction to compute).
- Lesson: Representing "no limit" as `0` is a clean sentinel — `if (gthDrawingTime === 0)` is unambiguous. The pill's `data-val="0"` → `parseInt('0')` → `0` wires correctly. `gthSyncSettingsUI` uses `String(gthDrawingTime)` which equals `'0'` matching the correct pill.

**Design: Auto-start timers on intake and shrink-intro screens (IMPLEMENTED)**
- Intake countdown: 30s → 15s auto-start. Auto-advances by calling `btn-gth-intake-ready.click()` on expiry.
- Shrink-intro countdown: New 15s auto-start on the host/single-device path only. Clients see no countdown (their button is already hidden). Auto-advances by calling `btn-gth-shrink-start.click()` on expiry. Timer cleared immediately if host taps the button manually. Timer cleared in `gthResetState()`.
- Timer variable: `gthShrinkIntroTimer` added to state declarations and cleared in both `gthResetState()` and `resetToLobby()` in `engine.js` (engine teardown entry for GTH).
- Lesson: Auto-start timers on gate screens are a soft nudge that prevents dead lobbies without being heavy-handed. The host-only guard on the shrink-intro timer is essential — clients would otherwise auto-tap a hidden button and trigger duplicate `GTH_PHASE2_BEGIN` broadcasts.

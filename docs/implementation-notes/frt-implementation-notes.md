...
- **`FRT_LEAF` (green) was never actually FRT's colour — reused by habit into the menu heading (8 Sep 2026, superseding the "left deliberately unchanged" note in the 2 Aug 2026 recolour above).**
  **What happened:** the new suite-wide two-tone menu heading (see `ui-style.md` § Menu Title Treatment) initially coloured "Salad" with the game's existing `FRT_LEAF` constant (`#047857`, a green) because it was already the established "readable accent on white" colour — used for the How-to step label and the "Call TRUE" button. The owner caught it immediately: FRT's actual brand is yellow, and green has never meant anything to this game.
  **Root cause:** `FRT_LEAF` was picked once, long before this pass, purely because it was legible on white — nobody at that point asked "is this actually FRT's hue?" because there was no on-page text usage yet to expose the mismatch. Every later addition (the heading) then copied the existing constant forward by habit rather than deriving a new accent from the real brand hex.
  **Fix, round 1:** renamed `FRT_LEAF` → `FRT_ACCENT`, value changed `#047857` → `#A16207` (a darkened `yellow-700`) — chosen on hue-angle-and-contrast math (35.5°, 4.9:1 on white), neither of which caught that it reads as brown to the eye. Darkening pure yellow for legibility pushes it toward brown/olive far faster than most hues, because yellow is the lightest hue at full saturation to begin with — a "same hue family, darker" pick can clear both checks and still not look yellow.
  **Fix, round 2 (owner call):** stop tuning for legibility — `FRT_ACCENT = FRT_FILL` (`#FFE500`), the literal brand fill, at every site (How-to step label, "Call TRUE" button, selected-card outline, heading). Contrast cost accepted outright rather than negotiated.
  **Lesson:** two distinct mistakes here, not one. (1) Don't reach for whatever existing "it already has good contrast" constant is lying around without checking it's actually the game's colour — that's what put green there at all. (2) Passing a hue-angle-and-contrast check is not the same as looking at the swatch — pick a replacement, then look at it, especially for yellow. Generalised into `ui-style.md` § Menu Title Treatment, which also records that FRT is now the sanctioned exception to "always use a darkened accent" — every other game still should.

...
| Mouth | The tile(s) a port occupies on the board edge — DERIVED from `{ edge, idx, corner? }`. **Always two edge-units.** At a corner it takes one of two forms: **flush** (both units along one edge, starting at the corner — two tiles) or **wrapped** (`corner: true` — one unit on each of the two edges meeting there, both fronting the SAME single tile, drawn as an L). Narrowing a two-tile mouth to its open half is a legal build move; covering the whole mouth is not. It used to collapse to one unit at either end of an edge, which made a flush corner door unauthorable (v207, D43) |
| Corner cycle | Tapping a corner tile with the Ingress/Egress brush in the Node Editor **cycles** the door: wrapped → flush along the tapped edge → flush along the perpendicular edge → back. Away from a corner a tap offers one placement and is idempotent. This is the only way to reach all three corner doors without a drag gesture (v207) |
...
- Every port's mouth is TWO edge-units wide and never truncates: `idx` clamps to `len-2` and the mouth is `[idx, idx+1]`. The only one-tile mouth is the opt-in **wrapped** corner form (`corner: true`), whose second unit sits on the perpendicular edge and fronts the same tile. Calibrated against maze.game, which does exactly this — see `nt-implementation-notes` D43. `corner` is the only field ever added to a port record; every other property is still derived
...
- **A WRAPPED corner port cannot be narrowed at all** — its mouth is one tile, so there is no second half to give up. Any block covering it removes the only source/target, refusing the placement outright. This asymmetry is the mechanic, not a gap — and since v207 it is the author's *choice*, not the model's default: a **flush** corner door is two tiles and narrows like any other
...
- **Latency = travel distance × `NT_BASE_TILE_TIME` (1000 ms/tile) + a corner cost + duration-based slows.** The corner cost is `NT_TURN_COST` (235 ms) × `sin(θ/2)/sin(45°)` charged at each *maze* corner — the two port-mouth joints at either end are free, since the stub geometry forces those turns regardless of the maze. Calibrated tile-for-tile against maze.game (±4 ms across three reference boards): a taut polyline is **not** self-timing. See `nt-implementation-notes` D40 and `tools/nt-path-probe.js`
- **A honeypot slows on ENTRY to its AoE, and re-arms when the runner leaves** — 0.5× speed for `NT_HONEYPOT_DURATION` (30000), non-stacking (a re-trigger extends the window, never deepens the slow), AoE radius `3√2` from the block's centre. There is **no re-trigger cooldown**: a snaking route that crosses the same AoE four times is slowed four times, which is how maze.game's "quad" is scored. Fitted against five reference boards whose on-screen trigger counts are recorded in `maze-puzzles/boards/*.json` — every count reproduces exactly, and the five land within −3.2…+2.7% on score. See `nt-implementation-notes` D44 and `tools/nt-slow-fit.js`
- The cost is spread over `NT_TURN_BRAKE` (0.4 tiles) either side of the corner as a triangular braking ramp, so playback reads as braking into a bend rather than halting at each vertex. The per-substep charge is the exact integral of that ramp, so **the score does not depend on `NT_SUBSTEP`** — only the animation smoothness does
...
| `nt-logs-overlay` | Data (slide-up) | z-[90] | "// SYSTEM LOGS" — per-cycle SER comparison, file-explorer aesthetic. Reused (not duplicated) for Debug Mode's attempt log (v201) — own seat lists every attempt, another seat shows their synced best only. `#nt-logs-player-chips` (v202) lets you switch whose log is showing without closing the overlay — needed in PTP, where the log is otherwise stuck on whichever player finished last |
...
| `screen-nt-authoring` | Debug Mode's Node Editor — hand-author a Node (brush pills, Firewall/Honeypot Resources steppers, Randomise Topology / Randomise Resources — renamed from Randomise Terrain/Budget, v203, to fit NT's network-engineering register — Deploy Node) at the `w × h` chosen on Sandbox Initialisation; reached at session start (via Sandbox Initialisation's Deploy) and again on the Author New Node loop-back from the summary (which skips Sandbox Initialisation and reuses the same dimensions) |

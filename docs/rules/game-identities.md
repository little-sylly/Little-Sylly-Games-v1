...
### The Three-Column Stage (DD-12, supersedes DD-11's one-row layout)
The decision area is a CSS `grid grid-cols-3` — not a flex row — because grid is what lets columns 1 and 3 stretch to match column 2's height automatically. **Col 1, "on the table":** the Treat slot (art, or a dashed placeholder in the exact footprint a Treat will occupy — the same idea DD-11 used for the trail, generalised) over the Crumbs value, which moved here after briefly living in its own band above the row and, before that, in the private strip beside two personal numbers it had nothing in common with. **Col 2:** `just revealed`, unchanged — still the single largest image, `cjar-card-stage` (8.5rem). **Col 3, "the jar":** the face-down deck, sized up twice now (`cjar-card-next`, 6.5rem → **7.2rem**) with a bolder count, so its two stacked pieces reach col 2's height without the deck itself outgrowing the hero. Below the grid, the **history strip** runs full width — before the first flip it now shows a single dashed placeholder rather than dead space, the same fixed-footprint idea as the Treat slot.

Persistent standings (`cjarRenderRevealRows`) render in every phase, not only during reveal (DD-11), and now sit directly **above the action buttons**, which are fixed at the screen's floor. The private strip below them lost its two most-discussed chips: Cookie Stash and This Raid were removed outright, not restyled, because the standings table already shows both for the viewer's own seat regardless of Open Book (`cjarStashVisible()` is unconditionally true for the viewer's own index) — the strip now carries only Dibber Dobber's Favourite/Watcher/Owes chips and hides itself entirely in the base game.

...
- **Narrowing an opponent's door is now a legal build move.** A 2×2 firewall/honeypot block may cover ONE half of a two-unit mouth — the route re-routes through the still-open half. Covering BOTH halves (sealing the door) is refused, exactly like covering any other route-critical tile — `ntPathExists` is the single legality gate, with no port-specific special-casing

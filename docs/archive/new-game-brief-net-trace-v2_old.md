...
- Bad Sectors are randomly generated (both position and count) for all modes.
...
- **Native Honeypots** (map-generated): inherent to the Node layout seed. Baked into the environment before the build phase begins. Controlled by the `Native Honeypots` setting (0/1/2 = how many the seed *can* place; actual count is random up to that cap).
...
A Node can have **up to 4 total Honeypots** (e.g. 2 Native + 2 Allocated). This cap is adjustable via settings if needed later.
...
- **Bad Sector generation:** Need a scaling algorithm that keeps maps interesting and non-restrictive across 8/9/10 sizes (avoid generating an unsolvable or trivially open map).
...
- **Honeypot terminology:** Native (map-generated, controlled by setting 0/1/2) vs Allocated (player-deployed from inventory). ✓

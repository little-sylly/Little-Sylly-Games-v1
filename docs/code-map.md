...
| `CTL_BORDER` / `CTL_BORDER_FIRM` | The adaptive die-cut border (spec D8) — `0.055` art inset (the width) and `3.2` ring opacity (the firmness; it saturates rather than thickens). The border **is** the bump lip made visible: the lip exists only inside the sticker's own alpha, so on a matching shell it outlines a blank plateau. Tuned against the worst case the spec names, `flw` on FRT `#FFE500` at 1.01:1. **SW v230: painted only on a DARK shell/ear (`lum <= 0.5`) — on a light one the ring is skipped entirely** (`ctlStampShell` / `ctlRedrawEars`), the bump lip still stands proud either way. The sticker's own near-white die-cut edge was found to already read fine against a light surface without it |
...
| `ctlZoom` (SW v230) | View-only camera-distance multiplier, `CTL_ZOOM_MIN`–`CTL_ZOOM_MAX` (0.55–1.7); driven by wheel/pinch on the Workshop stage only (`ctlBindZoom`), never persisted, reset to `1` on every Workshop open/close |
| `ctlRedrawScheduled` (SW v230) | Coalesces a burst of redraw requests (a slider, several sticker images decoding at once) into one `requestAnimationFrame`-scheduled `ctlRedrawShell`/`ctlRedrawEars` — each of those re-uploads the full 2048² textures to the GPU, so this is what stops N events costing N uploads. See `ctlScheduleRedraw()`. **A live sticker drag no longer goes through this path at all** — see `ctlStickerDragPending` below, the redraw only happens once, on release |
| `ctlStickerDragPending` / `ctlDragGhostEl` (SW v230) | The drag preview: `ctlStickerDragPending` holds the last LEGAL placement patch a live drag would commit if released right now (`null` over an illegal spot); `ctlDragGhostEl` is the flat DOM ghost (`.ctl-drag-ghost`, lazily created inside `#ctl-stage` by `ctlEnsureDragGhost()`) that follows the pointer during the drag. Neither the real sticker nor the atlas texture is touched until release |
| `ctlStickerDragActive` (SW v230) | Set by `ctlBindPointer`'s `pointerdown` when the press lands on the **currently selected** placement; while true, `pointermove` repositions the sticker live instead of rotating the view |
| `ctlButtonPressSuppressesTap` (SW v230) | Set on a `pointerdown` that `ctlTryPress` consumed as a real button press while sticker mode was `'idle'` — read on the matching `pointerup` to stop that same press ALSO arming/selecting/placing whatever sticker sits under the button. Once something IS armed/selected, a tap on a button's spot is let through so it can still be placed there deliberately |
...
| `ctlStickerDispatch(action)` | The one impure seam: reduce, re-point **both** `ctlDraft.stickers` and `ctlDesign.stickers` at the new array, redraw both atlases, re-render the panel. Assigning the array rather than mutating it in place is what keeps "discard unsaved changes" honest. **`'adjust'` (SW v230) takes a separate, cheaper path** — `ctlScheduleRedraw()` instead of an immediate redraw, and no panel re-render at all, since a slider tweak or a live sticker drag changes neither the list nor the selection |
| `ctlScheduleRedraw()` (SW v230) | Coalesces `ctlRedrawShell`/`ctlRedrawEars`/`ctlWake` into one `requestAnimationFrame` callback regardless of how many callers ask for one first — used by `'adjust'` dispatches and by the sticker image loader's `onload`, both genuine burst sources (a slider drag; several stickers decoding on a cold lobby load) |
...
| `ctlStickerHitIsSelected(hit)` (SW v230) | Does this raycast hit land on `ctlStickerState.selected`? Checked on `pointerdown` to decide rotate-view vs drag-sticker |
| `ctlStickerDragTo(ev)` (SW v230) | One frame of the live drag — re-plans the selected placement's position exactly as a tap would (same legality, same refusal text via `CTL_REFUSAL`) but touches **only** `ctlDragGhostEl`: no dispatch, no texture re-upload, until release. `r` must be copied across explicitly when a drag crosses from the shell onto an ear (a shell placement carries no `r` field) |
...
`tools/visual-controller-stickers.js` (50 checks — real headless Chromium: the tab, the book, a
placement actually painting texels, the lobby ornament repainting with them, the light-shell/light-
ear border going away SW v230) and
...
**SW v230's interaction additions (randomise, zoom, drag-to-reposition, tap-to-book, reselect-not-
relocate) are presentation/interaction, not rules/packets/state — no new harness was written for
them** (`logic-engine.md` § Harness rule). Confirmed instead with a throwaway `visual-check` driver
each round (not committed): drag-to-reposition moves the selected placement with zero spurious
undo-history entries and leaves rotation untouched elsewhere (and, once reworked to a ghost preview,
fires zero dispatches during the drag itself, only one on release); wheel/pinch zoom clamps and
resets on close; `ctlScheduleRedraw()` collapses a 30-dispatch `'adjust'` burst to zero gallery
rebuilds; the rainbow gradient tracks a live `GAME_BRAND_HEX` edit; picking a placed sticker from
the book lands all four surface combinations (shell front/back, earL/earR) dead-centre on screen
after the tween settles, reduced motion snaps it in ~1 frame, and a manual drag or button press
starting mid-tween cancels it rather than fighting it. **The border change is different in kind** —
...
| `smGatewayStream()` | Builds the sequence via `smGatewayBuildLines()`, clears any prior `smTypewriterTimers`, then schedules `SM_GATEWAY_LINES` (70) staggered writes (`SM_GATEWAY_GAP` = 70ms apart, ~4.9s total — plays once, so it earns a proper length) into `#sm-gateway-log`, finishing with `smGatewayFinish()`. Has its own `smReducedMotion()` check — the global CSS block cannot reach a `setTimeout`, so under reduced motion the whole log is written at once and `smGatewayFinish()` runs immediately |
| `smGatewayStatLine()` | One line of the header's stat box (a fake module id + a fake load percentage) — two are generated once when `smOpenGateway()` opens the screen, written into `#sm-gateway-stat1`/`#sm-gateway-stat2` |
...
| `ntDrawPortMarker(grid, port, color, inward, w, h)` | Extracted port-marker draw (shared by build and authoring grids) — takes both dimensions since the rectangular-grid round; spans the port's whole MOUTH (`ntMouthIdxs`), not one tile. Since the corner-mouth round (v207) a WRAPPED corner port draws a second segment on the perpendicular edge, giving maze.game's L. Returns the primary segment |
| `ntDrawPortSegment(grid, edge, first, units, color, arrow, primary, w, h)` | One bar of a marker (v207). Primary segments carry the direction arrow and `.nt-port-marker`; a wrapped corner's second segment carries `.nt-port-wrap` and no arrow — so "how many ports are drawn" stays a count of markers, not of bars |
| `ntPortCorner(port, w, h)` | Corner descriptor for a WRAPPED mouth (v207) → `{ tx, ty, perpEdge, perpIdx, name }`; `null` for every other port. Derived from `{ edge, idx }` alone — a wrapped port's idx is pinned to one END of its edge, and an edge plus an end names exactly one corner |
| `ntPortCandidates(edge, idx, w, h)` | The placements a Node Editor tap offers, in cycle order (v207). One away from a corner; AT a corner three — wrapped, flush along the tapped edge, flush along the perpendicular edge |
...
| `ntMouthIdxs(port, w, h)` | The port's mouth, DERIVED from `{ edge, idx, corner? }`: **always two** tile indices along the port's own edge, `idx` clamped to `[0, len-2]`. Bounded per axis (top/bottom against `w`, left/right against `h`). An opt-in `corner: true` gives the WRAPPED form — one unit here, one on the perpendicular edge, both fronting the same tile, so it returns a single index. It used to collapse to one unit at either end of an edge, which made a door flush against a corner unauthorable (v207, `nt-implementation-notes` D43) |
| `ntPortHalves(port, w, h)` / `ntMouthTiles(port, w, h)` / `ntPortSubs(node, port)` | Resolved-port helpers — a **resolved port** is the same `{ edge, idx }` shape with `idx` pinned to one mouth half, which is what lets `ntPortMouth`/`Interior`/`Border`/`Outside`/`Sub` keep working unmodified. `ntMouthTiles` maps halves → tile coords; `ntPortSubs` → config-lattice sub-cells |
| `ntMouthsIntersect(a, b, w, h)` | TILE-SET intersection, not an idx comparison — two corner ports on DIFFERENT edges can share one physical tile (e.g. `left/0` and `top/0` are both tile `(0,0)`) |
| `ntResolveHalf(node, port, subCell)` | Maps a route's actual sub-cell endpoint back to the mouth half it used; falls back to the first half rather than returning `undefined` (a wrong-but-finite point is catchable, a NaN is not — D46's class) |
| `ntIsMouthTile(tx, ty)` | **Render classification only** since the two-unit-ports round — every placement-reservation caller was deleted when `ntPathExists` became the sole legality gate. Its one surviving caller is `ntCellType` |
...
| `ntAuthSetPort(tx, ty)` | Snaps Ingress/Egress to the nearest edge tile. At a CORNER a repeat tap **cycles** `ntPortCandidates` — wrapped → flush → flush (v207); elsewhere there is one candidate and the tap is idempotent. Each candidate is tried in turn rather than rejecting outright, so a corner form sealed by terrain falls through to one that routes. Enforces only that the two mouths are tile-set DISJOINT (`ntMouthsIntersect` — idx-equality alone misses near-miss overlaps and same-tile corners) and the node still routes — deliberately skips the generator's corner-proximity and different-edges re-rolls (an author's choice, not a bug) |
...
| `ntAuthRandomiseTerrain()` | "Randomise Topology" button. Re-rolls geometry AND ports via `ntGenerateNode(true, null, { w: ntNode.w, h: ntNode.h })` — a full re-roll (v203; previously restored the hand-authored Ingress/Egress pair if it still routed) that keeps the sandbox's own dimensions (v204 — passing `forcedDims` explicitly; without it `ntGenerateNode` defaults to a square `ntMatrixScale × ntMatrixScale` board) |
| `ntGenerateNode(keepInventory, forcedIngressIdx, forcedDims)` | Rolls a routable node — DNP and the standard match never pass `forcedDims` and stay square; only `ntAuthRandomiseTerrain` passes the sandbox's own `{w,h}` (v204, fixing a "one shared bound, two axes" latent bug — the same class as D44/D46/D48) |
| `ntAuthRandomiseBudget()` | "Randomise Resources" button. Rolls a budget using the same range expressions `ntGenerateNode` uses for its own roll |
...
| `ntComputeTimeline(polyline, honeypots)` | The timeline integrator. Distance × `NT_BASE_TILE_TIME`, **plus a `sin(θ/2)`-weighted corner cost** (`NT_TURN_COST`, maze corners only — polyline indices 3…len-4; port-mouth joints are free), spread over `NT_TURN_BRAKE` as a triangular braking ramp integrated exactly so the score is `NT_SUBSTEP`-independent. Honeypots fire **on ENTRY** to the AoE with hysteresis, re-arming when the runner leaves — there is no re-trigger cooldown (v207; the hysteresis is load-bearing, without it the fire count becomes a function of `NT_SUBSTEP`). Emits `samples[]` (broadcast in `NT_PLAYBACK` — finer substeps are taken **only** inside braking windows so the wire cost stays flat). Calibrated against maze.game — see `nt-implementation-notes` D40 (movement, v206) and D44 (slow, v207) |
| `ntResolveCycleMdlm(allPlacements)` | Host: computes timelines for all players (swapping `ntNode` + `ntMyPlacements` per player for DNP); derives SERs using cluster ceiling formula for DNP. **Debug MDLM branches straight to `ntShowSummary('match')` instead of `ntShowPlayback()`** — the `NT_PLAYBACK` SYNC still broadcasts, only the navigation it triggers changes; mirrored in the `NT_PLAYBACK` applier for clients (v201) |
...
| `ntShowSummary(mode)` | Shows `screen-nt-summary` with per-cycle SER leaderboard, or the final match report when `mode === 'match'`. Debug branch un-hides `btn-nt-logs-open` (v201) — Finish now lands here directly instead of hopping through playback first |
| `ntRenderSummary()` | Branches: solo (raw latency), **Debug MDLM (v201 — per-player table: name · tries · best latency, row tappable via `ntOpenLogs(i)`)**, **DNP (team Cycle Leader + two team cards with per-member contributions)**, and the flat per-player leaderboard for Standard multi-player |
| `ntDebugMySeat()` | Which seat is CURRENTLY live in `ntDebugAttempts` on THIS device — `mpMyPlayerIdx` in MDLM, `Math.max(0, ntPtpTurn - 1)` in solo/PTP (v201) |
| `ntDebugAttemptsFor(playerIdx)` | The full attempt history for one seat, or `null` if this device never held one — the live seat's `ntDebugAttempts` directly; in single-device PTP, any earlier seat's stash from `ntDebugAttemptsBySeat[playerIdx]`; MDLM has no stash (a device that was never that seat never held its attempts) (v203) |
| `ntRenderDebugLog(playerIdx)` | Debug's sibling of `ntRenderLogs` — one player's attempt history into `#nt-logs-content` instead of every cycle, via `ntDebugAttemptsFor(playerIdx)`. A seat this device holds a history for gets every attempt (newest first, capped at `NT_DEBUG_LOG_CAP` = 200); any other seat gets a single best-only row (v201). Calls `ntRenderDebugLogPills(playerIdx)` itself, so every entry point stays in sync |
| `ntRenderDebugLogPills(activeIdx)` | Populates `#nt-logs-player-chips` — one pill per player, active pill highlighted, click switches `ntRenderDebugLog(i)` in place without closing the overlay. Hidden for the standard (non-Debug) log and for solo (1 player). Fixes a real PTP gap: `ntDebugAttempts` is wiped at hand-over, so without a switcher the top-level System Logs button could only ever reach whichever player finished LAST (v202) |
| `ntOpenLogAttempt(playerIdx, attemptIdx)` | Plays back one Debug attempt — sibling of `ntOpenLogRound`, but for a single player's trace. Timeline is always recomputed via `ntComputeTimeline_local()`, never cached (`ntDebugAttempts` entries store `{ latencyMs, placements }` only) (v201) |
...
| `ntDebugFinish()` | Declares this player done — final, no un-finishing. Solo/PTP resolve inline and land on `ntShowSummary('match')` directly (v201 — was `ntShowComparisonPlayback()`); MDLM sets a slot in `ntDebugFinished` (host direct-updates its own slot per the dedup-guard rule, never self-sends) and resolves on `.every(Boolean)` via `ntResolveCycleMdlm` |
...
| `screen-comb-meadow` | **The board.** All seven phases live here — `h-screen` whitelist exception. Floating over the board stage's letterbox dead-space (zero layout height): `#comb-player-panel` (top-left, one row/seat, colour + name + this round's take from `combLastProduced`, active row lit = turn-handover signal — replaced `#comb-turn-order`), `#comb-roll-result` + `#comb-prob-ruler` (top-centre, the persistent landed roll + an 11-tick 2d6 frequency ruler), `#comb-player-strip` (bottom, 2×2 mini-cards — VP · cells · domes · walls · unplayed Instinct; taps → `combOpenMap('stats')`). `#comb-meadow-points` **removed** (`combPointStrip()` deleted) |
...
**`comb-map-overlay` is a 3-zone detail view** (SW v226, was a bare pinch-zoom board): opaque
`bg-[#1c1917]`; `#comb-map-top` (full-name player panel + big roll result + `combBuildAnnotatedRuler()` —
value + ×N/36 per column), `#comb-map-mid` (the board via `combDrawBoard`, gesture-locked:
`touch-action:none` + `overscroll-behavior:contain`, wheel + pinch zoom about centre, drag pan,
`combClampPan()`), `#comb-map-bottom` (`combRenderMapStats()` full table — VP · cells · domes · walls ·
chain · instinct · blossoms — + the status line, scrolls internally). Rendered from `combRepaintBoards()`
+ `combOpenMap()`; opened with `'stats'` from the player strip scrolls the bottom zone in.
...
| **UI (never serialised)** | `combPlacementMode` · `combLegalTargets` · `combPendingTarget` · `combMapOpen` · `combMapScrollTo` (`'stats'` when opened from the player strip) · `combZoom/PanX/PanY` (map overlay pan-zoom — now driven by wheel/pinch/drag, was fixed at 1) · `combLastProduced` (`N×5 | null` — last Scout Flight's public grid, drives the player panel's take column; cleared at cast + `combResetState()`) · `combRafHandle` · `combHowtoTab` · `combChainLen` (derived cache) · `combBuildPickerOpen` · `combTradeLive` · `combDraftGive/Want/To` · `combBankPick` · `combBloomPick` · `combOverflowPick` · `combPendingCardIdx` | the last seven are chunk 5's draft/picker state — composed in an overlay, never broadcast, never serialised |
| **Const** | `COMB_ROLL_FREQ` (2d6 counts out of 36) · `COMB_ROLL_RAMP` (6 colours, red→gold by distance from 7) | the probability ruler's tick heights + rarity ramp; pure lookup |
...
| `combRenderMeadow()` | ⚠️ The **only** writer to `screen-comb-meadow`, and it sets every element every time — seven phases share one screen. Calls `combRenderPlayerPanel` / `combRenderRollResult` / `combRenderProbRuler` / `combRenderPlayerStrip` for the floating zones |
| `combRenderPlayerPanel()` · `combRenderRollResult()` · `combRenderProbRuler()` · `combRenderPlayerStrip()` | The meadow's top/bottom floating zones. Pure, public state only. Panel is **colour dots only** (local player's ringed), no names. `combCountStructures(p)` → `{cells,domes,walls}` and `combStatIcon(kind)` (a ~14 px real-asset icon — board piece / deck back / generic blossom) back the strip and the map table |
| `combRenderMapPanel()` · `combRenderMapStats()` · `combBuildAnnotatedRuler()` | The map overlay's top + bottom zones — DOM-built (no innerHTML), rendered from `combRepaintBoards()` while the map is open. Stats headers are `combStatIcon` icons (VP/Chain stay text). `combClampPan(canvasEl)` bounds the pan gesture |
| `combFocusDimAlpha()` | 0, or the meadow-ground wash alpha (0.35 → board at ~65%): during a build placement, or after a roll over every non-blooming hex. Applied in `combDrawBoard` between the static blit and the pieces |
| `combDrawBlossomHints(ctx,tr)` · `combBlossomBloom(…)` · `combLighten(hex,amt)` | Trade Blossom reach affordance — a small **blinking beacon** (white-hot core + coloured halo, small radius, hard `combPulsePhase()` sin² pulse) on an unreached port's empty node or on your own piece once reached. Suppressed during placement |
| `combStartPulse()` · `combStopPulse()` · `combPulsePhase()` | The beacon's RAF (`combPulseRaf`, ~18 fps, the only meadow-screen animation). Started in `combShowMeadow()`; stopped in `combResetState()`, the quit handler and `combFinishMatch()`; self-stops off-screen. RAF not `setInterval` so it is inert under the mock. Reduced motion → no RAF, `combPulsePhase()` returns a steady 1 |
| `combRenderHex/Resource/Instinct/Piece` | The four render seams. Never build these primitives anywhere else |
| `combDrawBoard(canvasEl, viewport)` | Pure draw, target passed in — shared by the inline board and the magnifier so the map cannot become a second source of board truth. `combDrawTargets` glows in `COMB_PLAYER_COLOUR[combLocalIdx()]`, not the fixed brand gold |
...
⚠️ **The seven always broadcasts exactly one outcome** (SW v226 — fixed the strand where a 7 with
nobody over the carry limit advanced the host to `waspMove` and told the clients nothing, freezing
them in `overflow`; every 7 on the default Short Summer, which presets Overflow to Off). `combScoutFlight`'s
7 sends `COMB_ROLL_RESULT` with `seven:true` and **no `phase`**; `combBeginSeven()` then sends
`COMB_OVERFLOW_BEGIN` if a discard is owed, else `COMB_OVERFLOW_DONE {spilled:false, phase:'waspMove'}`.
The applier reads `('spilled' in p) ? !!p.spilled : true` and only logs *"The hive spilled over"* when true.
`combStatusLine`'s 7 is owe-aware.

**SYNC, host → all** (applied in `combHandleSync`): `COMB_MATCH_START` · `COMB_DRAFT_STATE` ·
`COMB_ROLL_RESULT` (`seven:true`, no phase, on a 7) · `COMB_OVERFLOW_BEGIN` · `COMB_OVERFLOW_DONE`
(`spilled` bool) · `COMB_WASP_PLACED` (⚠️ carries

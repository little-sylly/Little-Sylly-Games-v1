...
| `combDrawBlossomHints(ctx,tr)` · `combBlossomBloom(…)` | Trade Blossom reach affordance — a **soft breathing radial bloom** (no ring, no line) on an unreached port's empty node or on your own piece once reached, alpha pulsed by `combPulsePhase()`. Suppressed during placement |
| `combStartPulse()` · `combStopPulse()` · `combPulsePhase()` | The bloom's RAF (`combPulseRaf`, ~11 fps, the only meadow-screen animation). Started in `combShowMeadow()`; stopped in `combResetState()`, the quit handler and `combFinishMatch()`; self-stops off-screen. RAF not `setInterval` so it is inert under the mock. Reduced motion → no RAF, `combPulsePhase()` returns a steady 1 |

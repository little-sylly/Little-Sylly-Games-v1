...
| `combRenderPlayerPanel()` · `combRenderRollResult()` · `combRenderProbRuler()` · `combRenderPlayerStrip()` | The meadow's top/bottom floating zones. Pure, public state only. `combCountStructures(p)` → `{cells,domes,walls}` backs the strip and the map table |
| `combRenderMapPanel()` · `combRenderMapStats()` · `combBuildAnnotatedRuler()` | The map overlay's top + bottom zones — DOM-built (no innerHTML), rendered from `combRepaintBoards()` while the map is open. `combClampPan(canvasEl)` bounds the pan gesture |
...
| `combDrawBlossomHints(ctx,tr)` · `combBlossomDock(…)` | Trade Blossom reach affordance for the local player — a connector + node ring per port (faint on an unreached port's empty node, solid on your own piece once reached). Suppressed during placement |

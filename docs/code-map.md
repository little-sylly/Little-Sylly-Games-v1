...
| `ntDrawPortMarker(grid, port, color, inward, w, h)` | Extracted port-marker draw (pre-existing, shared by build and authoring grids) — takes both dimensions since the rectangular-grid round; spans the port's whole MOUTH (`ntMouthIdxs`) since the two-unit-ports round, not one tile |
...
| `ntMouthIdxs(port, w, h)` | Two-unit-ports round (18 Aug 2026) — the port's mouth is DERIVED from `{ edge, idx }`, no new field: two tile indices along the port's own edge, collapsing to one at either end. Bounded per axis (top/bottom against `w`, left/right against `h`) |
...
| `ntAuthSetPort(tx, ty)` | Snaps Ingress/Egress to the nearest edge tile; enforces only that the two mouths are tile-set DISJOINT (`ntMouthsIntersect`, since the two-unit-ports round — idx-equality alone misses near-miss overlaps and same-tile corners) and the node still routes — deliberately skips the generator's corner-proximity and different-edges re-rolls (an author's choice, not a bug) |

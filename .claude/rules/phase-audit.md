...
- [ ] **No split body/footer on centred screens** — grep the plugin for `getElementById('…-table-body')` and `getElementById('…-table-footer')` (or equivalent). If a screen puts content in a body div and action buttons in a separate footer div **and** the screen uses `min-h-screen` rather than `h-screen`, the split is wrong: the footer is detached at the bottom, the header is detached at the top, and nothing centres as a unit. Fix: move buttons into the body so all content is siblings inside the single inner wrapper. Split body/footer is only valid inside a `h-screen` sticky-footer screen. *[Elevated from frt-implementation-notes BUG-02, June 2026.]*
- [ ] **No `my-auto` as a vertical-centering workaround** — grep the plugin JS for `my-auto` in `className` strings. `my-auto` inside an `overflow-y-auto flex flex-col` container silently does nothing — the container's computed height equals its content height, leaving zero free space to distribute. This is the broken workaround that preceded the FRT fix. If `my-auto` is present as a centering attempt, the screen needs the section-level `flex items-center justify-center` pattern instead. `my-auto` is only meaningful when its parent has a fixed or explicit height with leftover space (e.g. centering one child inside an `h-screen` column — not the case here). *[Elevated from frt-implementation-notes BUG-02, June 2026.]*
